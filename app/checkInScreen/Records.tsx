...
const Records = () => {
    ...
    useEffect(() => {
        fetchCalculatedSpentCaloriesUnits();
    }, [])
    const handleSavePress = async() => {
        try {
            // console.log("Wellbeing:",wellbeing)
            // console.log("localtion:", loc);
            // console.log("activity:", act);
            // console.log("people:", peo);
            // console.log("text:", text);

            const token = await AsyncStorage.getItem('token');
            const checkinHealthInfo = {
                token: token,
                sleep: wellbeing.sleepValue,
                mood: wellbeing.moodValue,
                energy: wellbeing.energyValue,
                willPower: wellbeing.willPowerValue,
                places: loc,
                activity: act,
                people: peo,
                note: text
            }
            const res = await axios.post(`${process.env.EXPO_PUBLIC_BACKEND_URL}/api/v1/checkin/checkin-health-info`, checkinHealthInfo)
            if(res.data.success){
                {/* <Pressable onPress={() => router.push('homeScreen/checkIn')}> */}
                console.log(res.data.message);
                router.push('homeScreen/checkIn');
            }else{
                console.log(res.data.message);
            }
        } catch (error) {
    ...
    //api calls
    const fetchCalculatedSpentCaloriesUnits = async () => {
        try {
            const token = await AsyncStorage.getItem('token');
            const res = await axios.get(`${process.env.EXPO_PUBLIC_BACKEND_URL}/api/v1/checkin/checkin-drinkCalculation/${token}`)
            if (res.data.success) {
                setSpent(res.data.data.spent);
                setCalories(res.data.data.calories);
                ...
                >
                    <View style={Styles.container}>
                        <View style={Styles.container2}>
                            <Text style={Styles.text_style1}>£{spent.toPrecision(4)}</Text>
                            <Text style={Styles.text_style2}>Spent</Text>
                        </View>
                        <View style={Styles.container2}>
                            <Text style={Styles.text_style1}>{calories}</Text>
                            <Text style={Styles.text_style2}>Calories</Text>
                        </View>
                        <View style={Styles.container2}>
                            <Text style={Styles.text_style1}>{units}</Text>
